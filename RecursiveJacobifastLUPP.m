function [Q, D, flops, sweeps, sweep_OffNorm_history, new_break_flag] = ...
    RecursiveJacobifastLUPP(A, n_threshold, f, eps_threshold, recdepth, break_flag, ...
                              n3_ratio)
    % Recursive Jacobi Algorithm with recursive LUPP DDH07: James W. Demmel, Ioana Dumitriu, and Olga Holtz. "Fast linear algebra is stable." for the Symmetric Eigenproblem
    % Inputs:
    %   A - symmetric matrix
            ...
            for J = (I+b):b:n
                ...
                if normOffDiag(A_hat) >= eps_threshold
                    ...
                    [Q_hat, ~, sub_flops, sub_sweeps, ~, break_flag1] = ...
                        RecursiveJacobifastLUPP(A_hat, n_threshold, f, eps_threshold, ...
                                                  recdepth + 1, break_flag, ...
                                                  n3_ratio);
                    new_break_flag = break_flag1;
                    flops = flops + sub_flops;
    
                    % Perform recusrive LU decomposition on the b-by-2b submatrix
                    Q1 = Q_hat(1:min(b, size(Q_hat,1)), 1:min(2*b, size(Q_hat,2)));
                    %[~, ~, P] = lu(Q1');  % LU decomposition with pivoting on transposed submatrix 
                    [L, U, P, fastLU_flops] = fastLU(Q1');
                    
                    % Calculate and update FLOPs for recursive LU decomposition
                    m11 = size(Q1', 1); % Rows of Q1'
                    n11 = size(Q1', 2); % Columns of Q1'
                    flops_lu = fastLU_flops;
                    flops = flops + flops_lu;

                    % Apply permutation
                    Q_hat = Q_hat * P';
                    flops_perm = 2 * size(Q_hat, 1)^3;
