function [Q, D, flops, sweeps, sweep_OffNorm_history, new_break_flag] = ...
    RecursiveJacobiQRCP(A, n_threshold, f, eps_threshold, recdepth, break_flag, ...
                              n3_ratio)
    % Recursive Jacobi Algorithm with QRCP for the Symmetric Eigenproblem
    % Inputs:
    %   A - symmetric matrix
            ...
            for J = (I+b):b:n
                ...
                if normOffDiag(A_hat) >= eps_threshold
                    ...
                    [Q_hat, ~, sub_flops, sub_sweeps, ~, break_flag1] = ...
                        RecursiveJacobiQRCP(A_hat, n_threshold, f, eps_threshold, ...
                                                  recdepth + 1, break_flag, ...
                                                  n3_ratio);
                    new_break_flag = break_flag1;
                    flops = flops + sub_flops;
    
                    % Perform QRCP on the b-by-2b submatrix
                    Q1 = Q_hat(1:min(b, size(Q_hat,1)), 1:min(2*b, size(Q_hat,2)));
                    [Q2, ~, P] = qr(Q1);
                
                    % Swap m and n since here Q1 is a fat short matrix.
                    % The usual flops estimation 2mn^2-(2/3)n^3 for QRCP is for matrix of dimension m-by-n, where m > n.
                    n11 = min(b, size(Q1, 1));
                    m11 = min(2*b, size(Q1, 2));
                    flops_qr = 2 * m11 * n11^2 - (2 / 3) * n11^3;
                    flops = flops + flops_qr;
