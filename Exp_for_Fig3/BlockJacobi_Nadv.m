...
function [Q, D, flops, sweeps, sweep_OffNorm_history] = BlockJacobi_Nadv(A, blockSizes, eps_threshold, ordering, pivot_method, n3_ratio)
    % Block Jacobi Algorithm with non-adversarial bottom cases for Symmetric Eigenproblem
    % Inputs:
    ...
    while flops < n3_ratio * n^3
        ...
        for pairIdx = 1:size(blockPairs, 1)
            ...
            if normOffDiag(blockSubMatrix) >= eps_threshold
                ...
                % Here we substitue the original eig() with self-written non-adversarial classical Jacobi, which serves as a comparison to BlockJacobi_adv
                [U, ~, Nadv_flops, Nadv_sweeps, ~, adv_rotations] = classicalJacobi(blockSubMatrix, eps_threshold, "trig", Inf);
                flops = flops + Nadv_flops;
                sub_sweeps = sub_sweeps + Nadv_sweeps;
