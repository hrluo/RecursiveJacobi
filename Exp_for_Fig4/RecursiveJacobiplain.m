function [Q, D, flops, sweeps, sweep_OffNorm_history, new_break_flag] = ...
    ...
    % vanilla Recursive Jacobi Algorithm with pre-determined bottom case precision for the Symmetric Eigenproblem
    % Inputs:
    %   A - symmetric matrix
    ...
    %   bottom_case_precision - specified accuracy for the bottom-case eigenproblem, which is solved using classicalJacobi until this precision is reached.
    ...
    if n <= n_threshold
        ...
        % Replace MATLAB's eig() with a custom classical Jacobi method,
        % solved to a pre-specified bottom-case precision.
        [Q, D, sub_flops, ~, ~] = classicalJacobi(A, bottom_case_precision, "trig", Inf);
        D = diag(diag(D));
        ...
        for I = 1:b:(n-b+1)
            for J = (I+b):b:n
                ...
                if normOffDiag(A_hat) >= eps_threshold
                    ...
                    % Update total flops
                    flops = flops + 3 * 2 * n * size(A_hat, 1)^2;
                end
